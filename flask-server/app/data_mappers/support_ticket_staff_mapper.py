...
class SupportTicketStaffMapper:
    @staticmethod
    ...
    def get_tickets_by_staff_id(staff_id, db_session=None):
        ...
        db = db_session or get_db()
        cursor = db.cursor()
        cursor.execute("SELECT * FROM support_tickets WHERE staff_id = ?", (staff_id,))
    ...
    def create_ticket(data, db_session=None):
        ...
        cursor = db.cursor()
        statement = """
            INSERT INTO support_tickets (staff_id, order_id, subject, status, priority, assigned_to, created_at, updated_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """
